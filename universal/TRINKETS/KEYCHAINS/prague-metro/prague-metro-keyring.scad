...
module r_rectangle(r=5, a=20, h=1)
{
    a2 = a/2-r;
    hull()
    {
        translate([a2,a2,0]) cylinder(r=r,h=h);
        translate([a2,-a2,0]) cylinder(r=r,h=h);
        translate([-a2,-a2,0]) cylinder(r=r,h=h);
        translate([-a2,a2,0]) cylinder(r=r,h=h);
    }
...
module base()
{
    ...
    {
        r_rectangle (r=r,a=a,h=h);
        translate([0,0,-eps]) r_rectangle(r=r_,a=a_,h=h+eps2);
    }
}
...
module keychain()
{
    ...
    difference()
    {
        union()
        {
            base();
            color(black)
            translate([a-off,a-off,0]) cylinder(r=r, h=1);
        }
        translate([a-off,a-off,-eps]) cylinder(r=1.5,h=1+eps2);
        
    }
    
    // logo
    color("red")
    translate([0,-7.5,1]) scale([0.33,0.33, 0.7]) logo();
    
    // text
    color("red")
    translate([0,1,1-eps]) linear_extrude(0.7)
    text(text="METRO v Praze",  size = 2.4, font="Arial:style=Black", halign = "center");
}
