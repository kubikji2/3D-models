...
module cif_bracket()
{
    ...
    difference()
    {
        ...
        hull()
        {
            cubepp([2*lnk_middle_part_t,ci_a/2, ci_h], align="xZ");
            translate([0,0,_bd/2])
                cylinderpp(d=th_inner_d, h=2*lnk_middle_part_t,zet="x", align="x");
        }
        // nuthole
        translate([lnk_middle_part_t,0,_bd/2])
            rotate([0,-90,0])
            {
                //nut_hole(   d=lnk_fastener_d,
                //                standard=lnk_nut_standard, align="b");
        ...
        // cut
        translate([lnk_middle_part_t,0,lnk_middle_part_t])
        {
            cubepp([2*lnk_middle_part_t, _bd, _bd], align="xz");
            cylinderpp(r=lnk_middle_part_t, h=_bd, zet="y", align="x");
        }
    }
}

module camera_to_frame_interface()
{
    // baseplane    
    difference()
    {
