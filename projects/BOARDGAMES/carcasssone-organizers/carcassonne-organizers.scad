include<../../../lib/legacy/cubepp.scad>;
include<../../../lib/legacy/pattern-cuts.scad>;
    ...
    {   
        ...
        translate([cs_sx/2,cs_sy/2,0])
        {
            ...
            translate([0,0,+_h-eps])
                rotate([0,180,0])
                    scale([_sf,_sf,_h/10])
                        import("import/card-back-raw.stl");
        ...
        _off = 4;
        sx = [cs_sx-cs_sd, cs_swt+2*eps, sf_h-2*_off];
        translate([cs_sd/2,-eps,_off])
            hexagonal_pattern(s=sx, d=cs_sd, t=_off, vertical=true);
        ...
        // left
        sy = [cs_sy-cs_sd, cs_swt+2*eps, sf_h-2*_off];
        rotate([0,0,90])
