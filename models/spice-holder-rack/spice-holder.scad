...
module jar_holder_segment(is_first=false, is_last=false)
{
    ...
    difference()
    {
        ...
        body_x = is_first ? 2*vertical_step+spice_jar_spacing+spice_jar_d/2 - holders_clearance : (is_last ? last_segment_length : 3*vertical_step - 2*  holders_clearance);
        ...
        translate([spice_jar_offset,0,wt])
        {
            ...
            {
                translate([spice_jar_d/2+spice_jar_spacing,0,0])
                    cubepp([last_segment_length, drawer_width-2*wt, holder_height], align="xz");
            }
        }
...
// last segment
//translate([spice_jar_spacing+spice_jar_d/2+1*vertical_step + 2*vertical_step,0,0])
    jar_holder_segment(is_first=false, is_last=true);